        ...
        switch sweep_dirs(mod(s-1,sweep_dirs_len)+1)
            case 1
                for i=1:sz(2)
                    ...
                    if i~=sz(2)
                        rhs=rhs+coeff(:,i,5).*field(:,i+1);
                    end

                    if relax_factor==1
                        field(:,i)=tdma(-coeff(:,i,2),coeff(:,i,1),-coeff(:,i,3),rhs);
                    else
                        field(:,i)=field(:,i)*(1-relax_factor)+relax_factor*tdma(-coeff(:,i,2),coeff(:,i,1),-coeff(:,i,3),rhs);
                    end
                end
            case 2
                for i=sz(2):-1:1
            ...
            case 3
                for i=1:sz(1)
                    ...
                    if relax_factor==1
                        field(i,:)=tdma(-coeff(i,:,4),coeff(i,:,1),-coeff(i,:,5),rhs);
                    else
                        field(i,:)=field(i,:)*(1-relax_factor)+relax_factor*tdma(-coeff(i,:,4),coeff(i,:,1),-coeff(i,:,5),rhs);
                    end
                end
            case 4
                for i=1:sz(1)
