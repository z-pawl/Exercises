classdef TemperatureCondConv2DSteady < IComponent
    ...
    methods
            ...
            function coeff = get_coefficients(obj)
                ...
                % The density and the heat capacity is lineary interpolated using the properties of neighbouring cells
                % The velocity is taken as the velocity at the cell face
                % A - area of the cell face

                % The cell size vector is padded with the value 1e25 in order to ensure that the linear interpolation results in the value from inside of the domain
                % (val_b*1e25+0*dx_b)/(1e25+dx_b)=val_b as 1e25 is orders of magnitude greater than dx_b

                Fx = ([row0; obj.velocity.rho].*[obj.grid.dx; 1e25]+[obj.velocity.rho; row0].*[1e25; obj.grid.dx]) ...
                    .* ([row0; obj.cp].*[obj.grid.dx; 1e25]+[obj.cp; row0].*[1e25; obj.grid.dx]) ...
                    .* obj.velocity.vx .* obj.grid.dy ...
                    ./ ([obj.grid.dx; 1e25]+[1e25; obj.grid.dx]).^2;

                Fy = ([col0 obj.velocity.rho].*[obj.grid.dy 1e25]+[obj.velocity.rho col0].*[1e25 obj.grid.dy]) ...
                    .* ([col0 obj.cp].*[obj.grid.dy 1e25]+[obj.cp col0].*[1e25 obj.grid.dy]) ...
                    .* obj.velocity.vy .* obj.grid.dx ...
                    ./ ([obj.grid.dy 1e25]+[1e25 obj.grid.dy]).^2;



                % Conductance: D =  k/dx*A
                % dx - distance between the centers of neighbouring cells
                % The term k/dx is calculated as the parallel sum of the terms k/dx' for the neighbouring cells where dx' denotes the distance between the cell center and the cell face
                % Hence k/dx is equal to the harmonic mean of the terms k/dx'' for neighbouring cells where dx'' denotes the size of the cell in a direction normal to the cell face
                Dx = 2./([row0; obj.grid.dx./obj.k]+[obj.grid.dx./obj.k; row0]).*obj.grid.dy;
                Dy = 2./([col0 obj.grid.dy./obj.k]+ [obj.grid.dy./obj.k col0]).*obj.grid.dx;
                ...
                coeff(:,:,5) = coeff(:,:,1)+coeff(:,:,2)+coeff(:,:,3)+coeff(:,:,4)-obj.qt(:,:).*obj.grid.dx.*obj.grid.dy;
                % b = SC*dx*dy
                coeff(:,:,6) = obj.q(:,:).*obj.grid.dx.*obj.grid.dy;
            ...
            function coeff = apply_boundary_conditions(obj, coeff)
                ...
                [d_aP_west, d_b_west]=arrayfun(@(x) apply_boundary_condtion_cell(obj.boundaries_west(x,:), coeff(1,x,1), obj.grid.dx(1,1), ...
                    obj.grid.dy(1,x), obj.k(1,x), 1), 1:obj.sz(2));
                
                [d_aP_east, d_b_east]=arrayfun(@(x) apply_boundary_condtion_cell(obj.boundaries_east(x,:), coeff(end,x,2), obj.grid.dx(end,1), ...
                    obj.grid.dy(1,x), obj.k(end,x), -1), 1:obj.sz(2));
                
                [d_aP_south, d_b_south]=arrayfun(@(x) apply_boundary_condtion_cell(obj.boundaries_south(x,:),coeff(x,1,3),obj.grid.dy(1,1), ...
                    obj.grid.dx(x,1), obj.k(x,1), 1), 1:obj.sz(1));

                [d_aP_north, d_b_north]=arrayfun(@(x) apply_boundary_condtion_cell(obj.boundaries_north(x,:),coeff(x,end,4),obj.grid.dy(1,end), ...
                    obj.grid.dx(x,1), obj.k(x,end), -1), 1:obj.sz(1));
