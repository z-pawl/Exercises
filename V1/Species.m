classdef Species < IComponent
    properties
        ...
        D_f (1,1) function_handle = @(x) ones(x.sz)
        src_f (1,1) function_handle = @(x) zeros(x.sz)
        src_lin_f (1,1) function_handle = @(x) zeros(x.sz)
    ...
    methods
            function obj = Species(sim, grid, velocity, sz, Y, Cp, visc, k, M, V_D, tol, relaxation_factor)
                arguments
                    sim (1,1) Simulation2D
                    grid (1,1) Grid2D
                    velocity (1,1) IComponent
            ...
            function update_properties(obj)
                ...
                obj.p_partial = obj.Y .* obj.reactions.M_mix / obj.M .*obj.velocity.p;

                obj.D = obj.D_f(obj);
                obj.src = obj.src_f(obj);
                obj.src_lin = obj.src_lin_f(obj);
            end
            ...
            function coeff = get_coefficients(obj)
                ...
                % Conductance: D =  (rho*D)/dx*A
                % dx - distance between the centers of neighbouring cells

                Dx = Utilities.interpolate_center2faces(obj.velocity.rho,obj.grid.dx,1) ...
                    .* Utilities.interpolate_center2faces(obj.D,obj.grid.dx,1) ./ obj.grid.dx_stag .* area_x;

                Dr = Utilities.interpolate_center2faces(obj.velocity.rho,obj.grid.dr,2) ...
                    .* Utilities.interpolate_center2faces(obj.D,obj.grid.dr,2) ./ obj.grid.dr_stag .* area_r;
                ...
                coeff(:,:,5) = coeff(:,:,1)+coeff(:,:,2)+coeff(:,:,3)+coeff(:,:,4)-obj.src_lin(:,:).*volume+(Fx(2:end,:)-Fx(1:end-1,:))+(Fr(:,2:end)-Fr(:,1:end-1));
                % b = SC*dx*dy
                coeff(:,:,6) = obj.src(:,:).*volume;
            ...
            function coeff = apply_boundary_conditions(obj, coeff)
                ...
                function [d_aP, d_b] = apply_boundary_condtion_cell(bd_data, aB, dq, A, k, n_dir)
                    if bd_data(1)==1
                        % Dirichlet (given value) boundary condition
                        d_aP=0;
                        d_b=bd_data(2)*aB;
                    elseif bd_data(1)==2
                        % Neumann (given flux) boundary condition
                        d_aP=-aB;
            ...
            function converged = iterate(obj)
                ...
                obj.Y=obj.Y+obj.relaxation_factor*(solve(coeff,obj.Y,obj.num_of_iters,obj.sweep_dirs,2)-obj.Y);

                % Clamping the values of the mass fraction between 0 and 1
                obj.Y = clip(obj.Y, 0, 1);
            ...
            function res = calculate_residual(obj, coeff)
                ...
                res_vector = [row0; coeff(2:end,:,1).*obj.Y(1:end-1,:)] + [coeff(1:end-1,:,2).*obj.Y(2:end,:); row0] ... 
                    + [col0 coeff(:,2:end,3).*obj.Y(:,1:end-1)] + [coeff(:,1:end-1,4).*obj.Y(:,2:end) col0] ...
                    + coeff(:,:,6) - coeff(:,:,5).*obj.Y;
                % s_f = || [aP1*TP1; aP2*TP2; aP3*TP3; .... aPN-1*TPN-1; aPN*TPN] ||
                scaling_factor = norm(coeff(:,:,5).*obj.Y,1);
