...
vel=Velocity2DSIMPLE(sz,0.0*ones(sz+[1 0]),zeros(sz+[0 1]),zeros(sz),1,@(x) 1000*ones(sz), @(x) 1e-3*ones(sz), ...
...
for i=1:sz(2)
    vel.set_boundary_conditions("vx","west",i,[1,0.001,0]);
    vel.set_boundary_conditions("vx","east",i,[2,0,0]);

    vel.set_boundary_conditions("p","west",i,[1,101325,0]);
    vel.set_boundary_conditions("p","east",i,[2,0,0]);
end
% vel.set_boundary_conditions("p","west",1,[1,0,0]);
for i=1:sz(1)+1
    vel.set_boundary_conditions("vx","south",i,[2,0,0]);
