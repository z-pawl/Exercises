...
classdef Velocity2DSIMPLE < IComponent
    ...
    methods
        function obj = Velocity2DSIMPLE(sz, vx, vr, p, rho_f, visc_f, srcx_f, src_linx_f, srcr_f, src_linr_f, tol_vel, tol_pressure, rel_fact_v, rel_fact_p)
        ...
        function coeff = get_coefficients_vr(obj)
            ...
            coeff(:,:,4) = Dr(:,2:end).*Ar(:,2:end)-Fr(:,2:end).*(Fr(:,2:end)<0);
            % aP = aE+aW+aN+aS-SP*dx*dy
            coeff(:,:,5) = coeff(:,:,1)+coeff(:,:,2)+coeff(:,:,3)+coeff(:,:,4)-obj.src_linr(:,:).*volume+(Fx(2:end,:)-Fx(1:end-1,:))+(Fr(:,2:end)-Fr(:,1:end-1));
            % b = SC*dx*dy
        ...
        function p = apply_boundary_conditions_p_force(obj, p, is_pressure_correction)
            ...
            p_b_south=arrayfun(@(x) apply_boundary_condition_cell(obj.boundaries_south_p(x,:),obj.p(x,1),1,obj.grid.dr(1)), 1:obj.sz(1));

            p_b_north=arrayfun(@(x) apply_boundary_condition_cell(obj.boundaries_north_p(x,:),obj.p(x,end),-1,obj.grid.dr(end)), 1:obj.sz(1));
